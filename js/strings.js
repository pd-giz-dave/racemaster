...
export const PAGES = {
  ...
  'whats-new': `
    ...
    <ul>
      ...
      <li>Fixed frozen (sticky) columns on tables like Entries, Helpers and Mobile Files sometimes ending up
          overlapping the column right after them if the browser window was resized narrower after the page had
          already rendered</li>
      <li>Tables across the app now size their columns to fit their actual content, instead of stretching every
          column to fill the full window width whenever there's more room than the data needs</li>
      <li>Mobile Files' <strong>Poll server every</strong>, <strong>Skip races older than</strong> and
          <strong>Bluetooth logging</strong> options now sit on their own row, each in a clearly bordered box,
          above the action buttons</li>
      <li>Ticking <strong>Auto-update progress</strong> now immediately checks for anything missed while it was
          off, instead of waiting for the next change to notice</li>
      <li><strong>Safety Check</strong> now shows real time-of-day instead of elapsed race time: <strong>Last
          CP</strong> (Outstanding tabs), <strong>Start Time</strong> (Early Starters), and the new
          <strong>Retirees / DNFs</strong> tab's <strong>Where</strong>/<strong>When</strong> columns — preferring
          the phone's own clock reading whenever a sighting came from a mobile pull, falling back to converting
          elapsed time against the race start otherwise. A <strong>Race start</strong> / <strong>Time now</strong>
          line above the tabs, ticking live, makes it easy to judge how long ago that actually was</li>
      <li>Bib Allocations now also includes each entrant's <strong>category</strong>; also fixed "Skip races older
          than" sometimes mishandling a web-app-generated bib-allocations race's own age</li>
