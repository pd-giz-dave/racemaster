...
function currentRaceProgressContext() {
  ...
  const currentRaceCandidates = [COURSE.SENIORS, COURSE.JUNIORS]
    ...
    .filter(Boolean);
  // Phase 3: every device adopted via the Devices tab's own row tick (server marker or still-
  // pending local change — see js/mobile-files-adoption.js), matched by mule-ble.js against the
  // connected phone / its relay manifest by (deviceName, raceLabel).
  const adoptedTargets = buildAdoptedTargets(races, owner);
